...
run_loan_simulation <- function(n = 10000) {
    ...
    health_status <- sample(c("Good", "Fair", "Poor"), size = n, replace = TRUE, prob = HEALTH_PROBS)
    
    # Loan affordability rule
    can_afford_loan <- (income > LOAN_INCOME_THRESHOLD) & (credit_score > LOAN_CREDIT_THRESHOLD)
